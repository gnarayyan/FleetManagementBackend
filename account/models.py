...
class TestImage(models.Model):
    ...
    image = models.ImageField(upload_to=File(
        'test/').rename, null=False, blank=False)


class UserProfileModel(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE)
    avatar = models.ImageField(upload_to=File(
        'account/').rename, null=True, blank=True)
    role = models.CharField(max_length=1, choices=[(
        'A', 'Admin'), ('D', 'Driver'), ('H', 'Household User')], default='H')
    verification_status = models.CharField(
        max_length=1, choices=[('V', 'Verified'), ('U', 'Unverified')], default='U')
    municipality = models.ForeignKey(
        Municipality, on_delete=models.PROTECT, null=True)
    collection_route = models.ForeignKey(
        CollectionRoute, on_delete=models.PROTECT, null=True)
